...
class NeoPixel(adafruit_pixelbuf.PixelBuf):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            if power:
                try:
                    self._power = None #digitalio.DigitalInOut(power)
        ...
        self.pin = None #digitalio.DigitalInOut(pin)
        #self.pin.direction = None #digitalio.Direction.OUTPUT
        pygame.init()
        self.strip = stripsim.LEDStrip("ledstrip.yaml")
        self.strip.led_show()
    ...
    def _transmit(self, buffer: bytearray) -> None:
        ...
        #print(buffer, end='', flush=True)  # line ending
        #print(len(buffer))
        grb_buffer = [100]*(len(buffer))
        #GRB buffer
        i = 0
        for entry in buffer:            
            grb_buffer[i] = int(buffer[i])
            i += 1
        #print(f'i = {i} {grb_buffer}')
        self.strip.led_display_format(grb_buffer)
        self.strip.led_show()
